...
export default async (req, res) => {
  ...
  if (req.query.k === 'chat') {
    if (req.method === 'GET') {
      ...
      return res.json(Object.values(await redis.hgetall('chm') || {}).sort((a, b) => a.ts - b.ts).slice(-60));
    }
    const me = await whoami(req), b = req.body || {};
    if (!me) return res.status(401).json({ error: 'Log in again' });
    if (req.method === 'PATCH') {   // modifier son propre message
      const m = await redis.hget('chm', String(b.id)), text = String(b.text || '').trim().slice(0, 300);
      if (!m || m.name !== me || m.sticker || !text) return res.status(403).json({ error: 'Not allowed' });
    ...
    const msg = { id: Date.now().toString(36) + Math.random().toString(36).slice(2, 5), name: me, ts: Date.now() };
    if (b.sticker) {
    ...
    if (n > 300) {   // garde les 150 plus récents
      const old = Object.values(await redis.hgetall('chm')).sort((a, b) => a.ts - b.ts).slice(0, n - 150);
